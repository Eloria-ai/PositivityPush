...
class SupabaseService:
    ...
    async def get_weekly_reflection(self, user_id: str, week_start: str) -> Optional[Dict[str, Any]]:
        """Get user's weekly reflection data for specific week"""
        try:
            result = self.client.table("weekly_reflections") \
                .select("*") \
                .eq("subscriber_id", user_id) \
                .eq("week_start", week_start) \
                .execute()
            ...
            logger.error(f"Error getting weekly reflection for user {user_id}, week {week_start}: {e}")
            return None
    
    async def store_weekly_reflection(self, user_id: str, week_start: str, reflection_text: str, theme: str = None) -> bool:
        """Store user's weekly reflection response"""
        try:
            data = {
                "subscriber_id": user_id,
                "week_start": week_start,
                "reflection_response": reflection_text,
                "theme": theme,
                "created_at": "now()"
            }
            ...
            logger.info(f"Stored weekly reflection for user {user_id}, week {week_start}")
            return True
            
        except Exception as e:
            logger.error(f"Error storing weekly reflection for user {user_id}, week {week_start}: {e}")
            return False
...
"""
    ...
    # OLD WEEKLY ACCOUNTABILITY METHOD REMOVED
    # Weekly reflections now use introspective approach with weekly_reflections table
